        ...
        <div className='bg-slate-800 my-10 py-5 lg:py-10 px-10 rounded-xl'>
          <form action='' onSubmit={(e) => fetchData(e)} className='flex justify-center flex-wrap gap-6 lg:gap-16 items-center'>
            <div>
              ...
              <select onChange={(e) => setBase(e.target.value)} id="base" required className=" py-5 w-full font-semibold mb-3 border border-gray-300  rounded-lg block p-2.5">
                <option disabled>Choose a base</option>
              ...
              <input type="number" name="base" id="baseValue" required value={amount} min={1}
                onChange={(e) => setAmount(e.target.value)}
                className='text-lg h-11 w-full pl-4 rounded-lg' />
            </div>
            <div>
            <h3 className='text-white font-semibold mb-2'>To: </h3>
              <select onChange={(e) => setCurrency(e.target.value)} id="currency" required className="w-full font-semibold mb-3 py-5 border border-gray-300 rounded-lg block p-2.5">
